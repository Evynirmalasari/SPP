...
<table class="table table-striped table-bordered">
    ...
    foreach($query as $data){ 
        //echo "SELECT SUM (jumlah_bayar) as jumlah_bayar FROM pembayaran WHERE nisn='$data[nisn]'";
    ...
    <tr>
        ...
        <td>
            <?php
            if($kekurangan==0){
                echo"<span class='badge text-bg-warning'> Sudah Lunas </span>";
            }else{ ?>
                <a href="?url=tambah-pembayaran&nisn=<?= $data['nisn'] ?>&kekurangan=<?= $kekurangan ?>"class="btn btn-warning">Pilih & Bayar</a>

            <?php } ?>
        </td>
        <td>
            <a href="?url=history-pembayaran&nisn=<?= $data['nisn'] ?>" class="btn btn-danger" > History </a>
        </td>
    </tr>
    <?php  } ?>
</table>